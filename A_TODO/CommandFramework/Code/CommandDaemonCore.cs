namespace Prateek.CommandFramework
{
    using System;
    ...
    using Prateek.CommandFramework.Debug;
    using Commands.Core;
    ...
    public sealed partial class CommandDaemon : Daemon<CommandDaemon, CommandServant>, IDebugMenuNotebookOwner
    {
        #region Fields
        ...
        {
            foreach (CommandId register in commandReceiver.CallbackToRegister)
            {
                long noticeId = register.GetValidId();
                if (liveClients.TryGetValue(noticeId, out HashSet<CommandReceiver> receivers))
                {
        ...
        {
            foreach (CommandId register in commandReceiver.CallbackToUnregister)
            {
                if (liveClients.TryGetValue(register.GetValidId(), out HashSet<CommandReceiver> receivers))
                {
        ...
        {
            CommandServant servant = Instance.GetFirstAliveBranch();
            if (servant == null)
            {
                return;
            }
