...
namespace Prateek.Helpers {
    ...
    public class PersonalLoggerManager
    {
        ...
        public class GUISetup
        {
            ...
            public void Init()
            {
                ...
                if (BGBoxStyle == null)
                {
                    ...
                    BGBoxStyle.normal.background = Textures.Make(new Color(0, 0, 0, 0.8f), Color.black);
        ...
        public void DisplayGUI()
        {
            ...
            style.WindowRect.size = (Core.Code.CSharp.min(style.WindowRect.position + style.WindowRect.size, new Vector2(Screen.width, Screen.height)) - style.WindowRect.position);
            style.WindowRect.size = Core.Code.CSharp.min(style.WindowRect.size, style.WindowRect.size - (Core.Code.CSharp.max(style.WindowRect.position, 0) - style.WindowRect.position));
            style.WindowRect.position = Core.Code.CSharp.max(style.WindowRect.position, 0);

            //Build rects
            var buttonsRect    = new Rect(style.WindowRect.position + Core.Code.CSharp.max(Vector2.right * (style.WindowRect.width - buttonsSize.x), 0), buttonsSize);
            var headerRect     = new Rect(style.WindowRect.position, Core.Code.CSharp.max(Core.Code.CSharp.vec2(style.WindowRect.width - (buttonsRect.width + 10), headerSize.y), 0));
            var scrollViewRect = new Rect(style.WindowRect.position + Vector2.up * headerRect.height, Core.Code.CSharp.max(style.WindowRect.size - Vector2.up * headerSize.y, 0));
                ...
                if (m_loggers[i].logger != null && m_loggers[i].show.CanUse)
                {
                    ...
                    if (headerRect.width > minNameSize)
                    {
                        if (i >= style.BoxOffset)
                        {
                            ...
                            var nameSize   = Core.Code.CSharp.min(Core.Code.CSharp.max(style.ToolbarActive.CalcSize(content), minNameSize), headerRect.width).x;
                            ...
                            var offset = Core.Code.CSharp.min(Vector2.right * toggleRect.width, headerRect.width);
            ...
            windowPosition = Core.Code.CSharp.max(Core.Code.CSharp.min(windowPosition, new Vector2(Screen.width, Screen.height) - Vector2.one * headerRect.height * 4f), -(windowDefaultRect.size - Vector2.one * headerRect.height * 2f));
        ...
        public void Display(Rect rect, PersonalLoggerManager.RegisteredLogger logger, PersonalLoggerManager.GUISetup gui_setup)
        {
            ...
            logger.scroll = new Vector2(scrollPos.x, (scrollView.height - rect.height) - scrollPos.y);
            {
                ...
                foreach (var log in logHistory)
                {
                    ...
                    //DATA -----------------------------------------------
                    //Build log time
                    var timeStr = Format.Time(log.Timestamp, settings.LogTimeText, settings.LogTimeHourText);
                    ...
                    for (int i = 0; i < ownerCount; i++)
                    {
                        ...
                        if (owner != null)
                        {
                            ...
                            {
                                newObjName = Format.Color(newObjName, minorColor);
                        ...
                        if (i + 1 == ownerCount)
                        {
                            objName += atLeastOneNewOwner ? Format.Color(settings.LogNameSeparatorText, minorColor) : settings.LogNameSeparatorText;
                        }
                    }
                    lastLog.Owners.Copy(log.Owners);

                    //Build log text
                    logText = timeStr + taskStr + objName + Format.Color(log.Log.Build(), usedColor);
