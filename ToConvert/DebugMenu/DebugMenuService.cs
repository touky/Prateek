namespace Assets.Prateek.ToConvert.DebugMenu
{
    ...
    using Assets.Prateek.ToConvert.Input;
    ...
    public sealed class DebugMenuService : ServiceSingletonBehaviour<DebugMenuService, DebugMenuServiceProvider>
    {
        #region Static and Constants
        private static readonly float[] MAGIC_CONTENT_SIZE = {.5f, 0.5f};
        private static readonly string PREFS_MENU_TOGGLE = $"{typeof(DebugMenuService).Name}.MenuStatus";
        private static readonly string PREFS_SHORT_NAMES = $"{typeof(DebugMenuService).Name}.shortNamesStatus";
        private static readonly string PREFS_SCREEN_RATIO = $"{typeof(DebugMenuService).Name}.ScreenRatio";
        private static readonly string PREFS_SCREEN_SIDE = $"{typeof(DebugMenuService).Name}.ScreenSide";
        ...
        private DebugMenuNotebook notebook;
        ...
        {
            var tempNotebooks = new List<AbstractDebugNotebook>(notebooks);
            foreach (var notebook in tempNotebooks)
            {
                notebook.Unregister();
            }
        ...
        {
            InputService.RegisterLayer<DebugInputLayer>(() =>
            {
        ...
        protected override void OnRegister(DebugMenuServiceProvider provider)
        {
            base.OnRegister(provider);

            provider.AddDebugContent(notebook, rootPage);
        }
        ...
        {
            notebook = new MainDebugMenuNotebook("DBUG", "Debug Menu Service");
...
#endif
            notebook.AddPagesWithParent(rootPage, new LogLevelPage("DebugTools Log Level"));
            notebook.Register();
        }
        ...
        private void DrawGUI()
        {
            ...
            //eventsThisFrame.Add(Event.current.type);
            //Rect eventRect = new Rect(40, eventCount++ * 40, Screen.width, 40);
            //GUI.Box(eventRect, GUIContent.none, context.Background);
            //GUI.Label(eventRect, Event.current.ToString());
...
#endif
            {
                ...
                for (var c = 0; c < notebooks.Count; c++)
                {
                    var notebook = notebooks[c];
                    notebook.Draw(context);
                }

                var buttonSize  = Vector2.one * 2f * context.LineHeight;
                var titlesWidth = context.GetTitlesSize() * MAGIC_CONTENT_SIZE[UseShortNames ? 0 : 1];
                ...
                if (context.HasFields)
                {
...
#endif
                    {
                        ...
                        var buttonRect = GetButtonRect(screenRect);
...
#endif
                    {
                        var buttonRect = GetButtonRect(titlesRect);
        ...
        private Rect GetButtonRect(Rect screenRect)
        {
            ...
            var result     = new Rect(screenRect.position + (RightSideMenu ? 0 : screenRect.width) * Vector2.right, buttonSize);
            if (RightSideMenu)
            {
        ...
        {
            var buttonSize = Settings.buttonSize;
            var fontSize   = Settings.buttonFontSize;

            var extendFontSizeReference = context.Extend.fontSize;
            var closeFontSizeReference  = context.Close.fontSize;
        ...
        {
            #region Fields
            private DebugMenuService service;
            #endregion

            #region Properties
            public override bool IsActive
            {
                get { return service.IsOpen; }
            }
            ...
            #region Constructors
            public DebugInputLayer(DebugMenuService service)
            {
            ...
            {
                return service.titlesRect.Contains(cursorPosition)
                    || service.screenRect.Contains(cursorPosition);
            }
