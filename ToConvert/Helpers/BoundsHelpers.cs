namespace Assets.Prateek.ToConvert.Helpers
{
    ...
    public static class BoundsHelpers
    {
        ...
        public static void TrySimplifyBounds(List<Bounds> bounds, float encapsulateSkin = 0.05f)
        {
            ...
            for (var b0 = 0; b0 < bounds.Count - 1; b0++)
            {
                var bound0 = bounds[b0];
                for (var b1 = b0 + 1; b1 < bounds.Count; b1++)
                {
                    var bound1 = bounds[b1];
                    ...
                    var skinBounds = bound0;
                    skinBounds.Encapsulate(bound1);
                    if (!bound0.Contains(bound1) && !bound1.Contains(bound0))
                    {
                        var skinTestFailed = (skinBounds.extents - bound0.extents).magnitude > encapsulateSkin
