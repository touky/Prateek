namespace Assets.Prateek.ToConvert.Resources.ResourceTree
{
    ...
    public class ResourceTree<T> where T : ITreeLeafLocator
    {
        ...
        public void Store(T resource)
        {
            ...
            var             collection   = tagRegex.Matches(resource.Location);
            foreach (Match match in collection)
            {
                var tag = match.Groups[match.Groups.Count - 1].Value;
        ...
        {
            ResourceTree<T> nextBranch = null;
            foreach (var tags in identification.TreeTags)
            {
                var activeBranch = this;
        ...
        private static void RetrieveResources(ITreeIdentification identification, ResourceTree<T> tree, ITreeIdentificationResult identificationResult)
        {
            ...
            {
                foreach (var resource in tree.leaves)
                {
            ...
            if (tree.HasBranches)
            {
                foreach (var nextBranch in tree.branches.Values)
                {
