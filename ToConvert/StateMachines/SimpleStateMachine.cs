namespace Assets.Prateek.ToConvert.StateMachines
{
        ...
        where TTrigger : struct, IConvertible
    {
        ...
        protected SimpleStateMachine(ISimpleStateMachineOwner<TState, TTrigger> owner, string stateProfilerFormat = null, string triggerProfilerFormat = null)
        {
            ...
            var sortList = new List<TState>(initStates);
        ...
        protected virtual void GoToNextState()
        {
            if (currentState != nextState)
            {
                ...
                owner.StateChange(previousState, CurrentState);
            }
        }
        ...
        public void Advance()
        {
            if (IsRunning)
            {
                BeginFrame();
                {
                    owner.ExecuteState(CurrentState);
                }
                EndFrame();
...
#if ENABLE_PROFILER
            debug.InitProfiling(stateProfilerFormat, triggerProfilerFormat, (stateTexts, stateFormat, triggerTexts, triggerFormat) =>
            {
