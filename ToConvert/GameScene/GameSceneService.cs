namespace Assets.Prateek.ToConvert.GameScene
{
    ...
    using Assets.Prateek.ToConvert.GameScene.Messages;
    ...
    public sealed class GameSceneService : ResourceDependentService<GameSceneService, GameSceneServiceProvider>
    {
        ...
        private TaskLoadingMessage taskMessage;
        private GameSessionOpen activeSession;
        private AvailableScenes activeContainer;
        private int completedActions = NameRegex.Consts.INDEX_NONE;
        ...
        protected override void SetupCommunicatorCallback()
        {
            ...
            Communicator.AddCallback<GameSessionOpen>(OnGameSessionOpen);
            Communicator.AddCallback<GameSessionClose>(OnGameSessionClose);
            Communicator.AddCallback<GameLoadingGameplayNotice>(OnGameLoadingGameplay);
            Communicator.AddCallback<GameLoadingRestartNotice>(OnGameLoadingRestart);

            Communicator.AddCallback<LoadSceneRequest<LoadSceneResponse>>(OnLoadSceneRequestReceived);
            Communicator.AddCallback<UnloadSceneRequest<UnloadSceneResponse>>(OnUnloadSceneRequestReceived);
        }
        #endregion
        ...
        public void Add(SceneReference scene)
        {
            ...
            if (RegexHelper.TryFetchingMatches(scene.Loader.Location, RegexHelper.AddressTag, matches))
            {
                ...
                SessionDebugAvailable message = Message.Create<SessionDebugAvailable>();
        ...
        private void SendStatusMessage(LoadingTrackingStatus status, float progress = 1)
        {
            if (taskMessage == null)
            {
                taskMessage = Message.Create<TaskLoadingMessage>();
            }

            taskMessage.trackerState = new LoadingTaskTracker(GetType(), status)
            {
                StepProgress = progress
            };

            Communicator.Send(taskMessage);
        }
        ...
        private void OnGameSessionOpen(GameSessionOpen message)
        {
            activeSession = message;
        }

        private void OnGameSessionClose(GameSessionClose message)
        {
        ...
        private void OnGameLoadingGameplay(GameLoadingGameplayNotice message)
        {
            completedActions = NameRegex.Consts.RESET;
        ...
        private void OnLoadSceneRequestReceived(LoadSceneRequest<LoadSceneResponse> request)
        {
            ...
            {
                Communicator.Send(request.GetResponse());
