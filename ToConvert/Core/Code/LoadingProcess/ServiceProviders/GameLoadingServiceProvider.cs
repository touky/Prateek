namespace Mayfair.Core.Code.LoadingProcess.ServiceProviders
{
    ...
    public class GameLoadingServiceProvider : LoadingProcessServiceProvider
    {
        ...
        protected override void InternalInit(LoadingProcessService service)
        {
            ...
            TaskTrackingClearState taskClear = new TaskTrackingClearState(service);

            //Send notice
            NoticeState<GameLoadingPrerequisiteNotice> prereqNotice = new NoticeState<GameLoadingPrerequisiteNotice>(service.Communicator, Consts.WAIT_5_FRAMES);
            LoadingIdleState prereqIdle = new LoadingIdleState();

            //Send notice
            NoticeState<GameLoadingGameplayNotice> gameplayNotice = new NoticeState<GameLoadingGameplayNotice>(service.Communicator, Consts.WAIT_5_FRAMES);
            LoadingIdleState gameplayIdle = new LoadingIdleState();

            //Send notice
            NoticeState<GameLoadingFinalizeNotice> finalizeNotice = new NoticeState<GameLoadingFinalizeNotice>(service.Communicator, Consts.WAIT_5_FRAMES);
            LoadingIdleState finalizeIdle = new LoadingIdleState();

            //Send notice
            NoticeState<GameLoadingFinishedNotice> finishedNotice = new NoticeState<GameLoadingFinishedNotice>(service.Communicator);
            ...
            TaskTrackingClearState restartClear = new TaskTrackingClearState(service);
            NoticeState<GameLoadingRestartNotice> restartNotice = new NoticeState<GameLoadingRestartNotice>(service.Communicator, Consts.WAIT_5_FRAMES);
