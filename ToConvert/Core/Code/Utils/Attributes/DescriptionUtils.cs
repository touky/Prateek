namespace Mayfair.Core.Code.Utils.Attributes
{
    ...
    public static partial class DescriptionUtils
    {
        ...
        {
            return InternalGetDescription(value);
        }
        ...
        /// <param name="value">The value of the enum for which to return the description.</param>
        /// <returns>A description of the enum, or the enum name if no description exists.</returns>
        private static string InternalGetDescription(object value)
        {
            if (value == null)
                return null;
            ...
            {
                object[] enforcementAttributes = fieldInfo.GetCustomAttributes(typeof(DescriptionEnforcementAttribute), false);
                ...
                {
                    DescriptionEnforcementAttribute enforcement = (DescriptionEnforcementAttribute) enforcementAttributes[0];

                    if (enforcement.EnforcementType == DescriptionEnforcementAttribute.EnforcementTypeEnum.ThrowException)
                    {
                        throw new ApplicationException("No Description attributes exist in enforced enum of type '" + type.Name + "', value '" + value.ToString() + "'.");
                    }

                    return type.IsEnum ? type.GetEnumName(value) : value.ToString();
                }
